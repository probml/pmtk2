...
% 2007b). 
% 
% Currently, self loops are removed and not represented on the graph although
        ...
        % name property. 
            if(~ismember(layout.name,fieldnames(obj.layouts)))
                if(layout.isavailable())
                    ...
                    warning('GRAPHLAYOUT:layout','This layout is not available');
                end
            end
        end
    ...
    methods(Access = 'protected')
        ...
        % Process the inputs and perform error checking    
            ...
            if(~isequal(numel(obj.nodeLabels),size(adjMatrix,1),size(adjMatrix,2)))
                error('GRAPHLAYOUT:dimMismatch','The number of labels must match the dimensions of adjmatrix.');
            end
        ...
        % colors are specified, they are cycled through. 
            if(~isempty(nodeColors))
               ...
               if(numel(nodeColors) < obj.nnodes)
                   ...
                   nodeColors = nodeColors(1:obj.nnodes); %#ok
               end
               for i=1:obj.nnodes
