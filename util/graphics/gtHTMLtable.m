...
% HTML table
HTML = [HTML sprintf('<TABLE BGCOLOR=gray ALIGN=left  VALIGN="top">')];
% Table title row
if exist('NAME','var'),
  HTML = [HTML sprintf('<TR><TH COLSPAN=%g ALIGN=LEFT VALIGN="top" BGCOLOR=WHITE>%s</TH></TR>',szcols,NAME)];
end;
...
for ipage = 1:size(MAT,3),
  ...
  if (size(MAT,3) > 1),
    % display page line
    HTML = [HTML sprintf('<TR><TH COLSPAN=%g ALIGN=LEFT VALIGN="top" BGCOLOR=ivory>%s(:,:,%g)</TH></TR>',szcols,MATname,ipage)];
  end;
  % Column labels
  if ~isempty(COLS),
    ...
    if (iscell(COLS)),
      for jj = 1:length(COLS),
        HTML = [HTML sprintf(['<TH BGCOLOR=white ALIGN=LEFT VALIGN="top">' COLFORMAT '</TH>'],[COLS{jj}])];
      end;
    else
      HTML = [HTML sprintf(['<TH BGCOLOR=white ALIGN=LEFT VALIGN="top">' COLFORMAT '</TH>'],COLS)];
    end;
    HTML = [HTML sprintf('</TR>\n')];
  end;
  % format data rows
  for ii = 1:size(page,1),
    ...
    if ~isempty(ROWS),
      if ii<=length(ROWS),
        if (iscell(ROWS)),
          HTML = [HTML sprintf(['<TH BGCOLOR=white ALIGN=LEFT VALIGN="top">' ROWFORMAT '</TH>'],[ROWS{ii}])];
        else
          HTML = [HTML sprintf(['<TH BGCOLOR=white ALIGN=LEFT VALIGN="top">' ROWFORMAT '</TH>'],ROWS(ii))];
        end;
      else
    ...
    if (iscell(page)), % if data is of type cell array
      for jj = 1:length({page{ii,:}}), % columns of data
        ...
        HTML = [HTML sprintf(['<TD BGCOLOR=white ALIGN=right VALIGN="top">' tFORMAT  '</TD>'],page{ii,jj})]; % add data cell
      end;
      ...
      if (~isempty(COLOR) && isnumeric(MAT)),
        for icol = 1:size(page,2),
          ...
          HTML = [HTML sprintf(['<TD BGCOLOR=#%6s ALIGN=right VALIGN="top">' FORMAT  '</TD>'],color',page(ii,icol))]; 
        end;
      else
        HTML = [HTML sprintf(['<TD BGCOLOR=white ALIGN=right VALIGN="top">' FORMAT  '</TD>'],page(ii,:))]; 
      end;
    end;
    HTML = [HTML sprintf('\n</TR>')]; % close data row
  end;
