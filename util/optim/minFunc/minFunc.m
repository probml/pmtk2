    ...
    switch method
        ...
        case QNEWTON % Use quasi-Newton Hessian approximation
                ...
                if i == 1
                    ...
                    elseif qnUpdate == 3 % McCormick-Huang update
                        theta = 1;
                        phi = 0;
                        psi = 1;
                        omega = 0;
                        ...
                        H = H + t1/t2 - t3/t4;
                    elseif qnUpdate == 4 % Pearson-Huang update
                        theta = 0;
                        phi = 1;
                        psi = 0;
                        omega = 1;
                        ...
                        t4 = (psi*s + omega*H'*y)'*y;
                        H = H + t1/t2 - t3/t4;
                    elseif qnUpdate == 5 % Self-Scaling BFGS (version 2)
                        
                        % Choice of Broyden Class
                        phi = 1; % BFGS

                        % Choice of scaling factor
                        omega = 0; % Choice that works really well

                        gamma = (1-omega)*(s'*y)/(y'*H*y) + omega*(s'*(H\s))/(s'*y);
                        v = sqrt(y'*H*y)*(s/(s'*y) - (H*y)/(y'*H*y));
                        H = gamma*(H - (H*y*y'*H)/(y'*H*y) + phi*v*v') + (s*s')/(s'*y);
                        
                    elseif qnUpdate == 6 % Oren's Self-Scaling Variable Metric
