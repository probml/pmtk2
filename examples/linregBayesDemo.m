...
function helperBasis(varargin)
    ...
    m = LinRegConj('-lambda', 0.001, '-transformer', T, '-sigma2', sigma2);
    Dtrain = DataTableXY(xtrain, ytrain);
    m = fit(m, Dtrain);
    [ypredTest, ypredTestProb] =  inferOutput(m,DataTable(xtest));
    ...
    nsamples = 10;
    for s=1:nsamples
        ws = sample(m.paramDist.wDist);
        w0 = ws(1);  w = ws(2:end);
        ms = Linreg('-w', w(:), '-w0', w0, '-sigma2', sigma2, '-transformer', T);
        %keyboard
        [xtrainT, ms.transformer] = trainAndApply(ms.transformer, xtrain);
        ypred = inferOutput(ms, DataTable(xtest));
