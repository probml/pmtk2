...
if 0
  notYetImplemented('Model Selection for Logistic Regression');
else
...
model = Logreg('-prior','L2','-lambda', lambdaL2, '-labelSpace', [1,2], ...
...
[yhat, T] = computeMapOutput(model,DataTable(testData)); 
pclass1 = T(:,1);
...
model = Logreg('-prior','L1','-lambda', lambdaL1, '-labelSpace',[1,2], ...
  '-transformer', T, '-addOffset', false); 
model = fit(model,D);
...
supportVectors = X(abs(model.params.w) > 1e-5,:);
