...
[yhat, pred] = inferOutput(m,DataTable(X));                      % predict on the training examples using MLE
...
mBayes = LogRegLaplace('-transformer',T, '-lambda', 1e-3);
 
mBayes = fit(mBayes,D);

pw = getParamPost(mBayes);
for i=1:2
  pwi = infer(pw,'-query',Query(i));
  figure; plotPdf(pwi); title(sprintf('w%d',i));
end


%% Predict using Monte Carlo sampling of the Posterior Predictive
[yhat, pred] = inferOutput(mBayes,DataTable(X));
