...
transmat = [0.95  , 0.05;
           0.10  , 0.90];
transDist = FactoredDist(DiscreteDist(transmat'),'-ndistributions',2);
%% Distribution over Starting States      
pi = [0.5,0.5];
startDist = DiscreteDist(pi');
%% Create the Model
model = Hmm(obsModel,startDist,transDist);
%% Sample
...
% requesting the mode of the posterior. 
%
viterbiPath = computeFunPost(model,'-data',rolls,'-funcs','mode');

%%
% This is different than the sequence of most likely states, which we can obtain
% by calling inferLatent() and taking the max. We obtain both the smoothed
% and filtered estimates, for comparison. 

postQ = inferLatent(model,Query({'singles','singles'},'-modifiers',{'filtered','smoothed'}),rolls);
[filtered,smoothed] = deal(postQ{:});
maxmargF = maxidx(filtered);
maxmarg  = maxidx(smoothed);


%%
% We can also sample from the posterior using fowards filtering, backwards sampling,
% and compare the mode of these samples to the predictions above. 
postSamp = computeFunPost(model,'-data',rolls,'-funcs','sample');
%%
