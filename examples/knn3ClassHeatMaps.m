...
K = 10;
beta = 0.3;
model = ProbKnn('-K',K,'-invTemp',beta);
model = fit(model,DataTableXY(Xtrain,ytrain));
[yHat,p] = inferOutput(model,DataTable([X1(:),X2(:)]));
p = pmf(p);
for c=1:3
...
beta = 1;
model = fit(ProbKnn('-K',10,'-invTemp',0.2,'-localKernel','tricube'),DataTableXY(Xtrain,ytrain));
[yHat,pY] = inferOutput(model,DataTable([X1(:),X2(:)]));
p  = pmf(pY);
