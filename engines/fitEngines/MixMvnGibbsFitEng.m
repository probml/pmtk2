classdef MixMvnGibbsFitEng < FitEng    
    ...
    methods
        ...
        function eng = MixMvnGibbsFitEng(varargin)
        ...
        function [model,latent] = fit(eng,model,X)
            ...
            model.paramDist.mu = muS;
            model.paramDist.Sigma = sigmaS;
            model.paramDist.mixingWeights = mixS;
            K = numel(muS);
        ...
        function convergencePlot(eng,X)
            model = eng.model;
            assertTrue(~isempty(model.paramDist),'you must fit first');
            mu = model.paramDist.mu;
            Sigma = model.paramDist.Sigma;
            mixW = model.paramDist.mixingWeights;
            S = size(model.paramDist.mu{1}.samples,2);
            K = numel(model.mixtureComps);
            lognormconst = zeros(S,1);
            l = zeros(size(X,1),K,S);
            for s=1:S
                for k=1:K
...
function [muS, SigmaS, mixS, latentS] = fullGibbsSampleMvnMix(distributions, mixWeights, data,alpha, varargin)
    ...
    for itr=1:Nsamples
        ...
        logMixWeights = log(mixWeights + sqrt(eps));
        for k=1:K
            XC = bsxfun(@minus, data, mu(k,:));
            logRik(:,k) = logMixWeights(k) - 1/2*logdet(2*pi*Sigma(:,:,k)) - 1/2*sum((XC*inv(Sigma(:,:,k))).*XC,2);
        end
...
function [kn,vn,Sn,mn] = updatePost(covtype, k0, v0, m0, S0, xbar, SSn, SSXX, SSXX2, xi) %#ok
    if(SSn == 0)
...
function [muoutDist, SigmaoutDist, mixoutDist, latentoutDist] = processLabelSwitching(muDist, SigmaDist, mixDist, latentDist, X, varargin)
    ...
    while( ~fixedPoint )
        ...
        klscore(value) = mean(loss);  % should preallocate and grow as needed
        ...
        if(deltakl > 0)
            warning('MixMvnGibbsFitEng:ObjectiveError','Objective did not decrease.  Returning with last good permutation');
            fixedPoint = true;
            fail = true;
        end
