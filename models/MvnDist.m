  ...
  methods
    ...
    function logp = logPdf(model,D)
      %logp = computeLogPdf(model.infEng,model,D);
       X = unwrap(D);
      logp = log(mvnpdf(X, model.params.mu(:)', model.params.Sigma));
    end
    ...
    %{
    function r = plotRange(model)
      mu = model.params.mu; Sigma = model.params.Sigma;
      d = model.ndimensions;
      switch d
        case 1,
          r = [mu(1)-4*Sigma(1,1), mu(1)+4*Sigma(1,1)];
        case 2,
          r = [mu(1)-4*Sigma(1,1), mu(1)+4*Sigma(1,1), ...
            mu(2)-4*Sigma(2,2), mu(2)+4*Sigma(2,2)];
        otherwise
    ...
    function [postmu, logevidence] = softCondition(pmu, py, A, y)
      ...
      b = py.params.mu(:);
      postmu = postSigma*(A'*Syinv*(y-b) + Smuinv*pmu.params.mu(:));
